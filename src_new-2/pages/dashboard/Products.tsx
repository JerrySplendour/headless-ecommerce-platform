    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canCreate && currentTab === "collections" && (
          <button onClick={() => setShowCollectionModal(true)} className="btn btn-primary">
            + Create Collection
          </button>
        )}
      </div>
...
function CollectionFormModal({
  ...
  const saveMutation = useMutation({
    mutationFn: (data: any) =>
      collection ? productsAPI.updateCollection(collection.id, data) : productsAPI.createCollection(data),
    onSuccess: () => {
      onSuccess()
    },
  })
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto border border-gray-300 rounded p-3">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setFormData({
                          ...formData,
                          product_ids: formData.product_ids.filter((id:any) => id !== product.id),
                        })
                      }
                    }}
                  />
          ...
          <div className="flex gap-4">
            ...
            <button type="submit" disabled={saveMutation.isPending} className="flex-1 btn btn-primary">
              {saveMutation.isPending ? "Saving..." : collection ? "Update Collection" : "Create Collection"}
            </button>
          </div>
        </form>
      </div>
